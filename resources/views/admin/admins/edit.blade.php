            ...
            <div class="grid grid-cols-2 gap-2">
                @foreach($roles as $roleId => $roleName)
                    <label class="inline-flex items-center gap-2 text-slate-300">
                        <input type="checkbox" name="roles[]" value="{{ $roleId }}" class="rounded border-slate-600 bg-slate-800" {{ in_array($roleId, $adminRoleIds) ? 'checked' : '' }}> {{ $roleName }}
                    </label>
                @endforeach
            </div>
        </div>
        <div class="flex items-center gap-3">
            <button type="submit" class="px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700">تحديث</button>
            <a href="{{ route('admin.admins.index') }}" class="px-4 py-2 rounded-lg bg-slate-700 text-slate-200 hover:bg-slate-600">رجوع</a>
        </div>
    </form>
</div>
@endsection
