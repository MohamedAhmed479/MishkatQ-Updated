...
<div class="p-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">إدارة الأجزاء</h1>
            <p class="text-sm sm:text-base text-slate-600 dark:text-slate-400">الأجزاء من 1 إلى 30</p>
        </div>
        <a href="{{ route('admin.juzs.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-colors touch-manipulation @permClass('juzs.create')" @permDisabled('juzs.create')>
            ...
            <span class="hidden sm:inline">إضافة جزء</span>
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.juzs.show', $juz) }}" class="p-2 rounded-lg text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 @permClass('juzs.view')" title="عرض" @permDisabled('juzs.view')>
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    <a href="{{ route('admin.juzs.edit', $juz) }}" class="p-2 rounded-lg text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 @permClass('juzs.edit')" title="تعديل" @permDisabled('juzs.edit')>
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <form action="{{ route('admin.juzs.destroy', $juz) }}" method="POST" onsubmit="return confirm('هل تريد حذف هذا الجزء؟')" class="inline @permClass('juzs.delete')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" @permDisabled('juzs.delete') class="p-2 rounded-lg text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20" title="حذف">
