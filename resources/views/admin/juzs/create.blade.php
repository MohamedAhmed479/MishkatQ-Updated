...
@section('page-title', 'إضافة جزء')
@section('page-subtitle', 'إنشاء تعريف جزء جديد')

@section('content')
<div class="p-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">إضافة جزء</h1>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700">
        <form method="POST" action="{{ route('admin.juzs.store') }}" class="space-y-6">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div>
                    ...
                    <input name="start_verse_id" type="number" required class="w-full px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-base">
                </div>
                <div>
                    <label class="block text-sm mb-2 text-slate-700 dark:text-slate-300">نهاية (آية ID)</label>
                    <input name="end_verse_id" type="number" required class="w-full px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-base">
                </div>
                <div>
                    <label class="block text-sm mb-2 text-slate-700 dark:text-slate-300">عدد الآيات (اختياري)</label>
                    <input name="verses_count" type="number" min="1" class="w-full px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 text-base" placeholder="سيتم احتسابه تلقائياً إذا تركته فارغاً">
                </div>
            </div>
