...
@section('page-subtitle', 'تحديث بيانات المستخدم: ' . $user->name)
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-bold text-slate-800 dark:text-slate-100">تعديل المستخدم</h2>
                    <p class="text-sm text-slate-600 dark:text-slate-400">{{ $user->name }} • {{ $user->email }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center justify-between text-sm">
                <div class="flex items-center gap-4">
                    <div class="text-slate-600 dark:text-slate-400">
                        <span class="font-medium">رقم المستخدم:</span> #{{ $user->id }}
                    </div>
                    <div class="text-slate-600 dark:text-slate-400">
                        <span class="font-medium">تاريخ التسجيل:</span> {{ $user->created_at->format('d/m/Y H:i') }}
                    </div>
                </div>
            ...
            <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                        <span class="flex items-center gap-2">
                            ...
                            كلمة المرور الجديدة
                            <span class="text-xs text-slate-500 dark:text-slate-400">(اختياري)</span>
                        </span>
                    </label>
                    <input type="password" name="password" 
                           class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" 
                           placeholder="اتركه فارغاً لعدم تغيير كلمة المرور" />
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">اتركه فارغاً إذا كنت لا تريد تغيير كلمة المرور الحالية</p>
                    @error('password')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-4 pt-6 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <div>
                        ...
                        <input type="url" name="profile_image" value="{{ old('profile_image', $user->profile->profile_image ?? '') }}" 
                               class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" 
                    ...
                    <div>
                        ...
                        <input type="number" name="total_points" value="{{ old('total_points', $user->profile->total_points ?? 0) }}" min="0" 
                               class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" 
                               placeholder="0" />
                        @error('total_points')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="verses_memorized_count" value="{{ old('verses_memorized_count', $user->profile->verses_memorized_count ?? 0) }}" min="0" 
                               class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" 
                               placeholder="0" />
                ...
                <div class="space-y-4 pt-6 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <div>
                        ...
                        <select name="tafsir_id" 
                            ...
                            @foreach($tafsirs as $tafsir)
                                <option value="{{ $tafsir->id }}" {{ old('tafsir_id', $user->preference->tafsir_id ?? '') == $tafsir->id ? 'selected' : '' }}>
                                    {{ $tafsir->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="daily_minutes" value="{{ old('daily_minutes', $user->preference->daily_minutes ?? 30) }}" min="0" required 
                               class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" 
                               placeholder="30" />
                        @error('daily_minutes')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="sessions_per_day" value="{{ old('sessions_per_day', $user->preference->sessions_per_day ?? 2) }}" min="1" required 
                               class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" 
                               placeholder="2" />
                    ...
                    <div>
                        ...
                        <select name="current_level" required 
                            ...
                            <option value="beginner" {{ old('current_level', $user->preference->current_level ?? '') == 'beginner' ? 'selected' : '' }}>مبتدئ</option>
                            <option value="intermediate" {{ old('current_level', $user->preference->current_level ?? '') == 'intermediate' ? 'selected' : '' }}>متوسط</option>
                            <option value="advanced" {{ old('current_level', $user->preference->current_level ?? '') == 'advanced' ? 'selected' : '' }}>متقدم</option>
                        </select>
                        @error('current_level')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
        ...
        <div class="mt-6 p-4 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                    <div class="text-red-800 dark:text-red-300 font-medium mb-2">منطقة خطر</div>
                    ...
                    <form method="POST" action="{{ route('admin.users.destroy', $user) }}" class="inline" 
                        ...
                        @method('DELETE')
                        <button type="submit" 
                                class="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-medium transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
