...
<div class="p-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">تفاصيل الكلمة #{{ $word->id }}</h1>
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <a href="{{ route('admin.words.edit', $word) }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-medium rounded-lg transition-colors touch-manipulation @permClass('words.edit')" @permDisabled('words.edit')>تعديل</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            ...
            <div class="text-xl sm:text-2xl font-semibold text-slate-900 dark:text-slate-100 mt-2" dir="rtl">{{ $word->text }}</div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            ...
            <div class="text-lg sm:text-xl font-semibold text-slate-900 dark:text-slate-100 mt-2">{{ $word->verse->chapter->name_ar ?? '' }} — آية {{ $word->verse->verse_number ?? '' }} (ID: {{ $word->verse_id }})</div>
