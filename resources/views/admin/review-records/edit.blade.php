...
@section('title', 'تعديل تقييم مراجعة')

@section('content')
<div class="p-6">
	<div class="flex items-center justify-between mb-6">
		<div>
			<h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">تعديل تقييم مراجعة</h1>
			<p class="text-slate-600 dark:text-slate-400">تحديث تقييم أداء المستخدم</p>
		</div>
	</div>

	<form method="POST" action="{{ route('admin.review-records.update', $review_record) }}" class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
		@csrf
		@method('PUT')
			...
			<div>
				...
				<select name="spaced_repetition_id" class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent" required>
					@foreach($revisions as $rev)
						<option value="{{ $rev->id }}" {{ $review_record->spaced_repetition_id==$rev->id?'selected':'' }}>#{{ $rev->id }}</option>
					@endforeach
				</select>
			</div>
			<div>
				<label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">تاريخ التقييم</label>
				<input type="datetime-local" name="review_date" value="{{ optional($review_record->review_date)->format('Y-m-d\TH:i') }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent" required />
			</div>
			<div>
				<label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">التقييم (0-5)</label>
				<input type="number" name="performance_rating" min="0" max="5" value="{{ $review_record->performance_rating }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent" required />
			</div>
			<div>
				...
				<select name="successful" class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent" required>
					<option value="1" {{ $review_record->successful ? 'selected' : '' }}>ناجحة</option>
					<option value="0" {{ !$review_record->successful ? 'selected' : '' }}>غير ناجحة</option>
				</select>
			</div>
			<div class="md:col-span-2">
				<label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">ملاحظات</label>
				<textarea name="notes" rows="3" class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent">{{ $review_record->notes }}</textarea>
			</div>
		</div>
		<div class="mt-6">
