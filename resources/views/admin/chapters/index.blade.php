...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">إدارة السور</h1>
            <p class="text-sm sm:text-base text-slate-600 dark:text-slate-400">إدارة سور القرآن الكريم</p>
        </div>
        <a href="{{ route('admin.chapters.create') }}" 
           class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-colors duration-200 touch-manipulation @permClass('chapters.create')" @permDisabled('chapters.create')>
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
            </svg>
            <span class="hidden sm:inline">إضافة سورة جديدة</span>
            <span class="sm:hidden">إضافة سورة</span>
        </a>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6">
        <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-3 sm:gap-4">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-400 truncate">إجمالي السور</p>
                    <p class="text-lg sm:text-2xl font-bold text-slate-900 dark:text-slate-100">{{ number_format($totalChapters) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-3 sm:gap-4">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-400 truncate">السور المكية</p>
                    <p class="text-lg sm:text-2xl font-bold text-slate-900 dark:text-slate-100">{{ number_format($makkahChapters) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-3 sm:gap-4">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-400 truncate">السور المدنية</p>
                    <p class="text-lg sm:text-2xl font-bold text-slate-900 dark:text-slate-100">{{ number_format($madinahChapters) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-3 sm:gap-4">
                <div class="p-2 sm:p-3 bg-orange-100 dark:bg-orange-900/20 rounded-lg flex-shrink-0">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-400 truncate">إجمالي الآيات</p>
                    <p class="text-lg sm:text-2xl font-bold text-slate-900 dark:text-slate-100">{{ number_format($totalVerses) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700 mb-6">
        <form method="GET" action="{{ route('admin.chapters.index') }}" class="space-y-4">
            <!-- Search -->
            <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="flex flex-col sm:flex-row gap-2 sm:items-end">
                    <button type="submit" 
                            class="flex-1 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-colors duration-200 touch-manipulation">
                        تطبيق الفلاتر
                    </button>
                    <a href="{{ route('admin.chapters.index') }}" 
                       class="flex-1 px-4 py-2.5 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 font-medium rounded-lg transition-colors duration-200 touch-manipulation text-center">
                        إعادة تعيين
                    </a>
                </div>
            </div>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.chapters.show', $chapter) }}" 
                                       class="p-2 text-slate-600 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-colors duration-200 @permClass('chapters.view')" @permDisabled('chapters.view')
                                       title="عرض">
                                    ...
                                    <a href="{{ route('admin.chapters.edit', $chapter) }}" 
                                       class="p-2 text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors duration-200 @permClass('chapters.edit')" @permDisabled('chapters.edit')
                                       title="تعديل">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-4 space-y-4">
                    ...
                    <div class="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-4 border border-slate-200 dark:border-slate-600">
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 bg-emerald-100 dark:bg-emerald-900/20 rounded-xl flex items-center justify-center">
                                    <span class="text-lg font-bold text-emerald-600 dark:text-emerald-400">{{ $chapter->id }}</span>
                                </div>
                                <div>
                                    <h3 class="font-semibold text-slate-900 dark:text-slate-100">{{ $chapter->name_ar }}</h3>
                                    @if($chapter->name_en)
                                        <p class="text-sm text-slate-600 dark:text-slate-400">{{ $chapter->name_en }}</p>
                                    @endif
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.chapters.show', $chapter) }}" 
                                   class="p-2 text-slate-600 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded-lg transition-colors duration-200 touch-manipulation @permClass('chapters.view')" @permDisabled('chapters.view')
                                   title="عرض">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                ...
                                <a href="{{ route('admin.chapters.edit', $chapter) }}" 
                                   class="p-2 text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors duration-200 touch-manipulation @permClass('chapters.edit')" @permDisabled('chapters.edit')
                                   title="تعديل">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <span class="text-slate-600 dark:text-slate-400">مكان النزول:</span>
                                <div class="mt-1">
                                    @if($chapter->revelation_place === 'makkah')
                                        <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-400">
                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
                                            </svg>
                                            مكية
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900/20 text-purple-800 dark:text-purple-400">
                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
                                            </svg>
                                            مدنية
                            ...
                            <div>
                                <span class="text-slate-600 dark:text-slate-400">عدد الآيات:</span>
                                <p class="font-medium text-slate-900 dark:text-slate-100">{{ number_format($chapter->verses_count ?: 0) }}</p>
                            </div>
                            @if($chapter->revelation_order)
                            <div>
                                <span class="text-slate-600 dark:text-slate-400">ترتيب النزول:</span>
                                <p class="font-medium text-slate-900 dark:text-slate-100">{{ $chapter->revelation_order }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
