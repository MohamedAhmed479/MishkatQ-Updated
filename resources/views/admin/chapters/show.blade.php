...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">تفاصيل السورة</h1>
            <p class="text-slate-600 dark:text-slate-400">معلومات مفصلة عن السورة: {{ $chapter->name_ar }}</p>
        </div>
    ...
    <!-- Chapter Header Card -->
    <div class="bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-xl p-8 mb-6 text-white">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-6">
                <div class="w-20 h-20 bg-white/20 rounded-full flex items-center justify-center">
                    <span class="text-3xl font-bold">{{ $chapter->id }}</span>
                </div>
                <div>
                    <h2 class="text-3xl font-bold mb-2">{{ $chapter->name_ar }}</h2>
                    @if($chapter->name_en)
                        <p class="text-xl text-emerald-100">{{ $chapter->name_en }}</p>
                    @endif
                    <div class="flex items-center gap-4 mt-3">
                        ...
                        @if($chapter->revelation_order)
                            <span class="inline-flex items-center gap-2 px-3 py-1.5 bg-white/20 rounded-full text-sm font-medium">
                                ...
                                ترتيب النزول: {{ $chapter->revelation_order }}
                            </span>
                        @endif
                    </div>
                </div>
            ...
            <div class="text-right">
                <div class="text-4xl font-bold">{{ $chapter->verses_count ?: '?' }}</div>
                <div class="text-emerald-100">آية</div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400">عدد الآيات الفعلي</p>
                    <p class="text-2xl font-bold text-slate-900 dark:text-slate-100">{{ number_format($actualVersesCount) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400">تقدم الحفظ</p>
                    <p class="text-2xl font-bold text-slate-900 dark:text-slate-100">{{ number_format($memorizationProgressCount) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400">خطط الحفظ</p>
                    <p class="text-2xl font-bold text-slate-900 dark:text-slate-100">{{ number_format($planItemsCount) }}</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="p-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-700">
                        <span class="text-slate-600 dark:text-slate-400">رقم السورة:</span>
                        <span class="font-medium text-slate-900 dark:text-slate-100">{{ $chapter->id }}</span>
                    </div>
                    <div class="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-700">
                        <span class="text-slate-600 dark:text-slate-400">الاسم العربي:</span>
                        <span class="font-medium text-slate-900 dark:text-slate-100">{{ $chapter->name_ar }}</span>
                    </div>
                    @if($chapter->name_en)
                    <div class="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-700">
                        <span class="text-slate-600 dark:text-slate-400">الاسم الإنجليزي:</span>
                        <span class="font-medium text-slate-900 dark:text-slate-100">{{ $chapter->name_en }}</span>
                    </div>
                    @endif
                    <div class="flex items-center justify-between py-3 border-b border-slate-200 dark:border-slate-700">
            ...
            <div class="p-6">
                ...
                <div class="space-y-3">
                    <a href="{{ route('admin.chapters.verses', $chapter) }}" 
                       class="flex items-center gap-3 w-full p-4 bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/30 rounded-lg transition-colors duration-200">
                    ...
                    <a href="{{ route('admin.chapters.memorization-progress', $chapter) }}" 
                       class="flex items-center gap-3 w-full p-4 bg-green-50 dark:bg-green-900/20 hover:bg-green-100 dark:hover:bg-green-900/30 rounded-lg transition-colors duration-200">
                    ...
                    <a href="{{ route('admin.chapters.memorization-plans', $chapter) }}" 
                       class="flex items-center gap-3 w-full p-4 bg-purple-50 dark:bg-purple-900/20 hover:bg-purple-100 dark:hover:bg-purple-900/30 rounded-lg transition-colors duration-200">
    ...
    <div class="mt-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6">
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
                <h3 class="font-medium text-red-900 dark:text-red-100 mb-2">منطقة الخطر</h3>
