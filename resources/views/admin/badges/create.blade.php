...
@section('page-title', 'إضافة شارة جديدة')
@section('page-subtitle', 'إنشاء شارة تحفيز جديدة للمستخدمين')
...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800 dark:text-slate-100">إضافة شارة جديدة</h2>
                <p class="text-sm text-slate-600 dark:text-slate-400">املأ المعلومات المطلوبة لإنشاء شارة تحفيز جديدة</p>
            </div>
        </div>
            ...
            <form method="POST" action="{{ route('admin.badges.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" required 
                              class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" 
                              placeholder="أدخل وصف الشارة وشروط الحصول عليها">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="points_value" value="{{ old('points_value', 100) }}" min="0" required 
                           class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" 
                           placeholder="100" />
                ...
                <div class="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <div>
                        ...
                        <input type="number" name="criteria_threshold" value="{{ old('criteria_threshold', 1) }}" min="1" required 
                               class="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all" 
                               placeholder="1" />
        ...
        <div class="mt-6 p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
            <div class="flex items-start gap-3">
                ...
                <div class="text-sm">
                    <div class="text-blue-800 dark:text-blue-300 font-medium mb-1">ملاحظات حول أنواع المعايير:</div>
                    <ul class="text-blue-700 dark:text-blue-400 space-y-1">
                        <li><strong>عدد الآيات المحفوظة:</strong> إجمالي الآيات التي حفظها المستخدم</li>
                        <li><strong>الأيام المتتالية:</strong> عدد الأيام المتتالية في الممارسة والحفظ</li>
                        <li><strong>إجمالي النقاط:</strong> مجموع النقاط التي جمعها المستخدم</li>
                        <li><strong>المراجعات المثالية:</strong> عدد المراجعات التي حصل فيها على تقييم مثالي</li>
                    </ul>
                </div>
            </div>
        </div>
