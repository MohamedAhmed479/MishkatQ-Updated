...
<div class="p-4 sm:p-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">إدارة التسجيلات</h1>
            <p class="text-slate-600 dark:text-slate-400 text-sm sm:text-base">التسجيلات حسب القارئ والآية</p>
        </div>
        <a href="{{ route('admin.recitations.create') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-colors @permClass('recitations.create')" @permDisabled('recitations.create')>
            <span class="hidden sm:inline">إضافة تسجيل</span>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700 mb-6">
        <form method="GET" action="{{ route('admin.recitations.index') }}" class="space-y-4">
            <!-- Search URL -->
            <div class="w-full">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 pt-2">
                <button type="submit" class="w-full sm:w-auto px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-colors">تطبيق</button>
                <a href="{{ route('admin.recitations.index') }}" class="w-full sm:w-auto px-6 py-2.5 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 font-medium rounded-lg transition-colors text-center">إعادة تعيين</a>
            </div>
        </form>
    </div>
        ...
        @if($recitations->count() > 0)
            ...
            <div class="md:hidden divide-y divide-slate-200 dark:divide-slate-700">
                ...
                <div class="p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <!-- Reciter info -->
                    <div class="mb-3">
                        <span class="text-sm font-medium text-slate-700 dark:text-slate-300">القارئ: </span>
                        <span class="text-sm text-emerald-600 dark:text-emerald-400 font-medium">{{ $rec->reciter->reciter_name ?? '' }}</span>
                        <span class="text-xs text-slate-500 dark:text-slate-400">(ID: {{ $rec->reciter_id }})</span>
                    </div>

                    <!-- Verse info -->
                    <div class="mb-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                        <div class="text-sm text-slate-600 dark:text-slate-400 mb-1">
                            {{ $rec->verse->chapter->name_ar ?? '' }} - الآية {{ $rec->verse->verse_number ?? '' }}
                        </div>
                        @if($rec->verse)
                        <p class="text-sm text-slate-700 dark:text-slate-200 leading-relaxed" dir="rtl">{{ \Illuminate\Support\Str::limit($rec->verse->text, 120) }}</p>
                        @endif
                    </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-3 shadow-sm transition-colors group hover:shadow-md hover:border-emerald-200 dark:hover:border-emerald-600">
                        <div class="js-audio-player" data-src="{{ $fullAudioUrl }}">
                            <audio src="{{ $fullAudioUrl }}" preload="none"></audio>
                            <div class="flex items-center gap-3">
                                <button type="button" class="p-2 rounded-full bg-emerald-600 text-white hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-transform duration-200 group-hover:ring-2 group-hover:ring-emerald-300 group-hover:scale-105 flex-shrink-0" data-role="toggle" title="تشغيل/إيقاف">
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 20 20" data-icon="play"><path d="M6 4.5v11l9-5.5-9-5.5z"/></svg>
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5 hidden" fill="currentColor" viewBox="0 0 20 20" data-icon="pause"><path d="M6 4h3v12H6zM11 4h3v12h-3z"/></svg>
                                </button>
                                <div class="flex-1 min-w-0">
                                    <div class="h-2 sm:h-3 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden cursor-pointer transition-all group-hover:bg-emerald-50 dark:group-hover:bg-slate-600" data-role="bar">
                                        <div class="h-full bg-emerald-600 transition-all" style="width:0%" data-role="fill"></div>
                                    </div>
            ...
            <div class="border-t border-slate-200 dark:border-slate-700 px-4 sm:px-6 py-4">{{ $recitations->links() }}</div>
        @else
            <div class="p-8 sm:p-16 text-center">
                <div class="text-slate-500 dark:text-slate-400">لا توجد تسجيلات</div>
            </div>
        @endif
    </div>
</div>
