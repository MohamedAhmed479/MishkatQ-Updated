...
@section('page-title', 'إضافة تسجيل')
@section('page-subtitle', 'ربط القارئ بآية مع رابط الصوت')

@section('content')
<div class="p-6">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">إضافة تسجيل</h1>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-700">
        <form method="POST" action="{{ route('admin.recitations.store') }}" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @csrf
            <div>
                ...
                <select name="reciter_id" required class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100">
                    @foreach($reciters as $r)
                        <option value="{{ $r->id }}">{{ $r->reciter_name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div class="md:col-span-2">
                ...
                <input name="audio_url" type="url" required placeholder="https://..." class="w-full px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100">
            </div>
