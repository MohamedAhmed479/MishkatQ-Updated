...
<div class="p-4 sm:p-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">إدارة الآيات</h1>
            <p class="text-slate-600 dark:text-slate-400 text-sm sm:text-base">بحث بالآية والنص</p>
        </div>
        <a href="{{ route('admin.verses.create') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-colors @permClass('verses.create')" @permDisabled('verses.create')>
            <span class="hidden sm:inline">إضافة آية</span>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-sm border border-slate-200 dark:border-slate-700 mb-6">
        <form method="GET" action="{{ route('admin.verses.index') }}" class="space-y-4">
            <!-- Search Text -->
            <div class="w-full">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 pt-2">
                ...
                <a href="{{ route('admin.verses.index') }}" class="w-full sm:w-auto px-6 py-2.5 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 font-medium rounded-lg transition-colors text-center">إعادة تعيين</a>
            </div>
        </form>
    </div>
        ...
        @if($verses->count() > 0)
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.verses.destroy', $v) }}" method="POST" onsubmit="return confirm('هل تريد حذف هذه الآية؟')" class="inline @permClass('verses.delete')">
                                        ...
                                        <button type="submit" @permDisabled('verses.delete') class="p-2 rounded-lg text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/20" title="حذف">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0016.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                        </button>
                                    </form>
                                </div>
                            </td>
            ...
            <div class="md:hidden divide-y divide-slate-200 dark:divide-slate-700">
                ...
                <div class="p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <!-- Chapter info -->
                    <div class="mb-2">
                        <span class="text-sm font-medium text-slate-700 dark:text-slate-300">السورة: </span>
                        <span class="text-sm text-slate-600 dark:text-slate-400">{{ $v->chapter->name_ar ?? '' }}</span>
                    </div>

                    <!-- Verse text -->
                    <div class="mb-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                        <p class="text-sm text-slate-700 dark:text-slate-200 leading-relaxed" dir="rtl">{{ Str::limit($v->text_uthmani, 120) }}</p>
                    </div>

                    <!-- Meta info -->
                    <div class="flex flex-wrap gap-2 text-xs">
                        <span class="inline-flex items-center px-2 py-1 rounded-md bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">
                            الجزء: {{ $v->juz_number }}
                        </span>
                        <span class="inline-flex items-center px-2 py-1 rounded-md bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">
                            الصفحة: {{ $v->page_number }}
                        </span>
                    </div>
                </div>
                @endforeach
            ...
            <div class="border-t border-slate-200 dark:border-slate-700 px-4 sm:px-6 py-4">{{ $verses->links() }}</div>
        @else
            <div class="p-8 sm:p-16 text-center">
                <div class="text-slate-500 dark:text-slate-400">لا توجد آيات</div>
            </div>
        @endif
    </div>
</div>
